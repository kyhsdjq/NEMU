...
cmd_handler(cmd_si)
{
    ...
    //     for (int i = 0; i < 8; i++)
    // 	{
    // 		printf("%02x ", instr_fetch(cpu.eip + i + 8, 1));
    // 	}
    // 	printf("\n");
		exec(1);
		return 0;
...
void ui_mainloop(bool autorun)
{
	if (autorun)
	{
        ...
        float cnt_ori = (float)cache_cnt.cnt_ori;
        float cnt_cache = (float)cache_cnt.cnt_cache;
        printf("cnt_ori = %d, cnt_cache = %d \n", cache_cnt.cnt_ori, cache_cnt.cnt_cache);
        printf("hit_rate = %f \n", ((cnt_cache - cnt_ori) * memory_read_cnt) / ((cache_read_cnt - memory_read_cnt) * cnt_ori));
#endif
#endif
